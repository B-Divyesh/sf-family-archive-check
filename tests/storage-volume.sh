...
loop_device="$(as_root losetup --find --show "$image_path")"
...
if as_root mount -t exfat "$loop_device" "$mount_path"; then
  :
elif command -v mount.exfat-fuse >/dev/null 2>&1; then
  as_root mount.exfat-fuse "$loop_device" "$mount_path"
else
...
if [[ "$actual_source" != "$loop_device" && "$actual_source" != *"$loop_device"* ]]; then
  echo "Expected the mounted source to be the exFAT loop device, got $actual_source" >&2
  exit 1
fi
