...
| exFAT | Ubuntu hosted runner | `bash tests/storage-volume.sh exfat` | a freshly formatted, `fsck.exfat`-validated exFAT image is attached as a loop device, mounted, and checked with `findmnt`; if the runner lacks the kernel driver, `exfat-fuse` mounts that same device. Native `scan()` inventories, validates, and hashes all five fixtures. |
