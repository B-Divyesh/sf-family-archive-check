...
|---|---|---|---|
| APFS | macOS hosted runner | `bash tests/storage-volume-macos.sh` | `diskutil` identifies APFS; native `scan()` inventories, validates, and hashes all five fixtures. |
...
| exFAT | Ubuntu hosted runner | `bash tests/storage-volume.sh exfat` | a fresh exFAT loopback image is mounted and checked with `findmnt`; native `scan()` inventories, validates, and hashes all five fixtures. |
